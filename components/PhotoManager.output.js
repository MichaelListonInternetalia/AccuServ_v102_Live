  ...
  customElements.define("photo-manager", function (_HTMLElement) {
    ...
    _createClass(PhotoManager, [{
      ...
      value: function editPhotoHandler(e) {
        ...
        var id = targ.getAttribute("data-photo-id");
        var that = document.getElementsByTagName('photo-manager')[0];
        var detail = { "id": id, "photosContainer": cti.store.jobs.selectedJob.photos };
        if (cti.fnNavigatePhotoDetail !== undefined) {
          cti.fnNavigatePhotoDetail(detail);
        } else {
      ...
      value: function _maxPhotosTaken() {
        ...
        var photosTaken = 0;
        for (var k in cti.store.jobs.selectedJob.photos) {
          if (category === undefined || category == null || category.toLowerCase() == cti.store.jobs.selectedJob.photos[k].category.toLowerCase()) {
            photosTaken++;
          }
        }
      ...
      value: function _render() {
        ...
        if (cti.store.jobs.selectedJob.photos !== undefined) {
          for (var k in cti.store.jobs.selectedJob.photos) {
            ...
            if (displayCategory === undefined || displayCategory === null || displayCategory.toLowerCase() == cti.store.jobs.selectedJob.photos[k].category.toLowerCase()) {
      ...
      value: function _renderPhoto(photoObject) {
        ...
        */
        ...
        dateInfo.innerHTML = '<span>Date</span>' + window.accuServFunctions.fnLocaleTimeString(photoObject.date);
